...
function initialize() {
    ...
    try {
        if (!gameCanvas || !recordPianoCanvas || !gameCtx || !recordPianoCtx) { console.error("FEIL: Nødvendige canvas/contexts mangler!"); return; }
        setupCanvases(); buildKeyMappings(); drawPianos(); populateSongSelector(); setupEventListeners(); resetUI(); switchTab('play'); updateRecordModeUI();
...
function switchTab(tabName) { console.log("Bytter til fane:", tabName); const isPlayTab = tabName === 'play'; if (!playArea || !recordArea || !tabButtonPlay || !tabButtonRecord) { console.error("Mangler fane/knapp-elementer!"); return; } playArea.classList.toggle('active', isPlayTab); recordArea.classList.toggle('active', !isPlayTab); tabButtonPlay.classList.toggle('active', isPlayTab); tabButtonRecord.classList.toggle('active', !isPlayTab); if (isPlayTab && isRecording) stopRecording(); if (!isPlayTab && isPlaying) { stopSoundPlayback(); pauseSongVisuals(); } setupCanvases(); buildKeyMappings(); drawPianos(); }
...
async function handleRecordPianoMouseDown(event) { if (!await ensureAudioInitialized()) { console.error("Lyd feilet."); return; } if (audioContext.state === 'suspended') await audioContext.resume(); const keyName = getKeyAtRecordCoords(recordPianoCanvas, event); if (!keyName) return; playFeedbackTone(keyName); if (recordingMode === 'step' && !isRecording) { selectedStepNote = keyName; addStepNoteButton.disabled = false; drawRecordPiano(); } else if (recordingMode === 'realtime' && isRecording) { console.log("TODO: Realtime note ON:", keyName); } }
