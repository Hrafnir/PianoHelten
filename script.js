...
const availableSongs = {
    ...
    "odetojoy.json": "Ode to Joy (Beethoven)", // Eksempel
    "pink_panther_theme.json": "Pink Panther Theme" // Eller den beste du fant fra MIDI
};
...
function initialize() {
    ...
    try {
        if (!gameCanvas || !recordPianoCanvas || !gameCtx || !recordPianoCtx) {
             console.error("FEIL: Nødvendige canvas-elementer eller contexts mangler ved initialisering!");
             // Vis en feilmelding til brukeren?
             document.body.innerHTML = '<h1 style="color:red; text-align:center; margin-top: 50px;">Kritisk feil: Kunne ikke laste applikasjonen. Sjekk konsollen.</h1>';
             return;
        }
        setupCanvases();
        buildKeyMappings();
        ...
        document.body.innerHTML = `<h1 style="color:red; text-align:center; margin-top: 50px;">Kritisk feil under initialisering: ${error.message}. Sjekk konsollen.</h1>`;
    }
}
...
function setupCanvases() {
    ...
    if (playContainer && gameCanvas) { gameCanvas.width = playContainer.clientWidth; gameCanvas.height = playContainer.clientHeight; }
...
function switchTab(tabName) {
    ...
    if (playArea) playArea.classList.toggle('active', isPlayTab); else console.error("playArea mangler");
    if (recordArea) recordArea.classList.toggle('active', !isPlayTab); else console.error("recordArea mangler");
    if (tabButtonPlay) tabButtonPlay.classList.toggle('active', isPlayTab); else console.error("tabButtonPlay mangler");
    if (tabButtonRecord) tabButtonRecord.classList.toggle('active', !isPlayTab); else console.error("tabButtonRecord mangler");

    if (isPlayTab && isRecording) stopRecording();
    if (!isPlayTab && isPlaying) { stopSoundPlayback(); pauseSongVisuals(); }
    if (!isPlayTab) { setupCanvases(); buildKeyMappings(); drawRecordPiano(); } // Oppdater record-visning
}
...
async function handleRecordPianoMouseDown(event) { console.log("RecordPiano MouseDown"); if (!await ensureAudioInitialized()) return; if (audioContext.state === 'suspended') await audioContext.resume(); const keyName = getKeyAtRecordCoords(recordPianoCanvas, event); if (!keyName) return; playFeedbackTone(keyName); if (recordingMode === 'step' && !isRecording) { selectedStepNote = keyName; addStepNoteButton.disabled = false; drawRecordPiano(); } else if (recordingMode === 'realtime' && isRecording) { console.log("TODO: Realtime note ON:", keyName); } }
function handleRecordPianoMouseUp(event) { console.log("RecordPiano MouseUp"); if (recordingMode === 'realtime' && isRecording) { console.log("TODO: Realtime note OFF"); } }
function startRecording() { console.log("Start Recording"); if (isRecording) return; isRecording = true; jsonOutputTextarea.value = ""; recordedRawNotes = []; recordedNotes = []; currentStepTime = 0; selectedStepNote = null; startRecordButton.disabled = true; stopRecordButton.disabled = false; clearRecordButton.disabled = true; recordModeSelector.disabled = true; recordTitleInput.disabled = true; recordArtistInput.disabled = true; recordTempoInput.disabled = true; if (recordingMode === 'realtime') { recordingStartTime = performance.now(); recordingStatusSpan.textContent = "Spiller inn..."; } else { recordingStatusSpan.textContent = ""; addStepNoteButton.disabled = true; addRestButton.disabled = false; drawRecordPiano(); } }
function stopRecording() { console.log("Stop Recording"); if (!isRecording) return; isRecording = false; startRecordButton.disabled = false; stopRecordButton.disabled = true; clearRecordButton.disabled = false; recordModeSelector.disabled = false; recordTitleInput.disabled = false; recordArtistInput.disabled = false; recordTempoInput.disabled = false; recordingStatusSpan.textContent = ""; addStepNoteButton.disabled = true; addRestButton.disabled = true; if (recordingMode === 'realtime') quantizeRecordedNotes(); generateJsonOutput(); drawRecordPiano(); }
function clearRecording() { console.log("Clear Recording"); if (isRecording) stopRecording(); recordedRawNotes = []; recordedNotes = []; jsonOutputTextarea.value = ""; currentStepTime = 0; selectedStepNote = null; clearRecordButton.disabled = true; drawRecordPiano(); }
function quantizeRecordedNotes() { console.warn("quantizeRecordedNotes() - Logikk mangler!"); recordedNotes = []; /* TODO: Implementer kvantisering basert på recordedRawNotes */ }
