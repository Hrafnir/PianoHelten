...
function switchTab(tabName) { console.log("Bytter til fane:", tabName); const isPlayTab = tabName === 'play'; if (!playArea || !recordArea || !tabButtonPlay || !tabButtonRecord) { console.error("Mangler fane/knapp-elementer i switchTab!"); return; } playArea.classList.toggle('active', isPlayTab); recordArea.classList.toggle('active', !isPlayTab); tabButtonPlay.classList.toggle('active', isPlayTab); tabButtonRecord.classList.toggle('active', !isPlayTab); if (isPlayTab && isRecording) stopRecording(); if (!isPlayTab && isPlaying) { stopSoundPlayback(); pauseSongVisuals(); } // *** Sørg for at begge pianoer tegnes etter bytte ***
 setupCanvases(); // Oppdater størrelser FØR mapping/tegning
 buildKeyMappings(); drawPianos(); // Tegn begge pianoene i sin nye tilstand }
...
function drawSpecificPiano(ctx, canvasElement, pianoHeightPx, mappingObject, activeHighlightKeys, highlightColor) { if (!ctx || Object.keys(mappingObject).length === 0) { console.warn(`Skipping drawSpecificPiano for ${canvasElement?.id}: no ctx or mapping`); return; } const pianoDrawHeight = canvasElement.height - pianoHeightPx; const blackKeyActualHeight = pianoHeightPx * blackKeyHeightRatio; ctx.clearRect(0, 0, canvasElement.width, canvasElement.height); // *** Tøm ALLTID før tegning ***
...
function gameLoop() {
    ...
    // *** Viktig: Bruk riktig context ***
    if (!gameCtx) { console.error("gameLoop: gameCtx er null!"); return; }

    // *** drawSpecificPiano tømmer nå, så vi kaller DEN FØR drawFallingNotes ***
    // gameCtx.clearRect(0, 0, gameCanvas.width, gameCanvas.height); // Fjernet

    // Tegn piano FØRST (inkludert tømming og highlights fra forrige frame)
...
function drawFallingNotes(currentBeat) {
    ...
    if (fallHeight <= 0) return; // Unngå deling på 0 eller negativ høyde
    ...
    currentSong.notes.forEach(note => {
        ...
        const notePixelHeight = Math.max(1, note.duration * pixelsPerBeat); // Sørg for min. høyde
        const yTop = yBottom - notePixelHeight;
        const xPosition = keyData.x;
        const noteWidth = keyData.width;

        // Sjekk og sett aktive taster (som før)
        ...
        if (yTop < gameCanvas.height && yBottom > 0) {
            ...
            gameCtx.beginPath(); // *** Viktig: Start ny path for hver note ***
...
function scheduleSongAudio() { /* ... Som før (med full planlegging) ... */ }
